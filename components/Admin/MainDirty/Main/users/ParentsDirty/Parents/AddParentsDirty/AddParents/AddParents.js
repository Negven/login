...
import Classes from './AddParents.module.css'
...
export default function AddParents(props) {
    ...
    const handleChangeSecondName = name => event => {
      ...
      const handleChangeMail = name => event => {
        setValues({ ...values, [name]: event.target.value });
        props.stateFunc.changeMail(event.target.value)
      };
      const handleChangePhones = name => event => {
        setValues({ ...values, [name]: event.target.value });
        props.stateFunc.changePhones(event.target.value)
      };
            ...
            <div className={Classes.popUpEdit}>
                ...
                <div className={Classes.blockTeach1}>
                    ...
                    <div className={Classes.teachLineBtn}>
                        <div className={Classes.saveBtnTech} 
                            onClick={() => {props.addParent(props.parent); 
                            props.closeAddParent()}}
                        >
                            <div className={Classes.textBtn}>Зберегти</div>
                        </div>
                        <div className={Classes.closeBtnTech} onClick={props.closeAddParent}><div className={Classes.textBtn} onClick={props.closeDeleteState}>Закрити</div></div>
                        <div></div>
                    </div>
                </div>
            </div>
