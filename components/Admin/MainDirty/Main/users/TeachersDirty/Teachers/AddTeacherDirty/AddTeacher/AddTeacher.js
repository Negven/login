...
export default function AddTeacher(props) {
    ...
    const handleChangeSecondName = name => event => {
      ...
      const handleChangeProfession = name => event => {
        setValues({ ...values, [name]: event.target.value });
        props.stateFunc.changeProfession(event.target.value)
      };
      const handleChangeAccessChats = name => event => {
        setValues({ ...values, [name]: event.target.value });
        props.stateFunc.changeAccessChats(event.target.value)
      };
            ...
            <div className={Classes.popUpEdit}>
                ...
                <div className={Classes.blockTeach1}>
                    ...
                    <div className={Classes.teachLineBtn}>
                        <div className={Classes.saveBtnTech} onClick={() => {props.addTeacher(props.teacher); props.closeAdd()}}><div className={Classes.textBtn}>Зберегти</div></div>
